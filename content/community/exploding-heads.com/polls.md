---
title: "Polls" 
name: "polls"
description: "- Posts must have a clear and concise question in the title. You may elaborate in the post body.- If Opinion Poll gives a list of options, do not make top-level comments different from those options.- Top-level comments must be short and single option. Usually a single word is enough. You may reply to your answer to add more info.- Do not repeat answers. Use the up and down vote buttons. Vote as many answers as you want.- Do not post loaded questions or push agendas.- Do not repost polls in less than a month.- Follow the Exploding Heads Rules."
type: community
date: "2023-06-16 11:06:38"
draft: false
nsfw: false
actor_id: "https://exploding-heads.com/c/polls"
host: exploding-heads.com
lastmod: "{[ .Updated }}"
subscribers: 5
posts: 4
comments: 4
weight: 4
dau: 2
wau: 3
mau: 3
instances:
- "exploding-heads_com"
tags: 
- "exploding-heads_com"

---

# /c/Polls on exploding-heads.com

[[home](/)]
[[/c/polls 🌎](https://exploding-heads.com/c/polls)]
[[exploding-heads.com](/instances/exploding-heads_com)]


## Description 

<blockquote class="description">
- Posts must have a clear and concise question in the title. You may elaborate in the post body.<br>- If Opinion Poll gives a list of options, do not make top-level comments different from those options.<br>- Top-level comments must be short and single option. Usually a single word is enough. You may reply to your answer to add more info.<br>- Do not repeat answers. Use the up and down vote buttons. Vote as many answers as you want.<br>- Do not post loaded questions or push agendas.<br>- Do not repost polls in less than a month.<br>- Follow the Exploding Heads Rules.
</blockquote>


## Details

| Activity | Count  |
|----------------------|---|
| Subscribers          | 5 |
| Posts                | 4  |
| Comments             | 4  |
| Daily Active Users   | 2  |
| Weekly Active Users  | 3  |
| Monthly Active Users | 3  |

<br>

| Activity | Date |
|----------------------|---|
| Published            | 2022-07-24 15:09:12 |
| Updated              | 2022-08-06 19:44:06 |
| Last Scraped         | 2023-06-16 11:06:38 |
