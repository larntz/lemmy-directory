---
title: "Political Compass Memes" 
name: "pcm"
description: "### Rules**R1**:Content posted must be legal in the United States.**R2**:Abide by community rules. Post authentic content into communities where you have a personal interest, and do not cheat or engage in content manipulation (including spamming, vote manipulation, ban evasion, or subscriber fraud) or otherwise interfere with or disrupt Based Count communities.**R3**:NSFW material is allowed so long as it is legal and properly marked. (I'm not moderating porn on the bus you twats)**R4**: All posts must relate to the political compass.  Meta Content and Quality OC relating to the subreddit is allowed.Orange by itself isn't PCM and must be accompanied by another quadrant.**Notes**Not everything with 4 colored squares is a PCM. So called “Get out of my head” posts are not allowed. "
type: community
date: "2023-07-06 18:29:38"
draft: false
nsfw: false
actor_id: "https://forum.basedcount.com/c/pcm"
host: forum.basedcount.com
lastmod: "2023-06-30 12:44:17"
published: "2023-06-16 16:04:05"
subscribers: 95
posts: 26
comments: 150
weight: 26
dau: 1
wau: 16
mau: 29
instances:
- "forum_basedcount_com"
tags: 
- "forum_basedcount_com"

---
